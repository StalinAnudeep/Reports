...
<div class="row row-cards row-deck">
	      <form class="card" action="oldcolonyrdng" method="post">
                <div class="card-body">
                  <h3 class="card-title"><strong><span class="text-danger">HT119</span> - Old Colony Readings </strong></h3>
                  <div class="row">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                      <div class="form-group">
                        <label class="form-label">GET Old Colony Readings</label>
                        <button type="submit" class="btn btn-success">GET Old Colony Readings</button>
                      </div>
                    </div>
                  </div>
                </div>
				...
				<thead>
					<tr>
						<th>S.NO</th>
							...
							<th>OPEN DATE</th>
							 <th>END DATE</th>
                            <th>KVAH OPEN READING</th>
                             <th>KVAH CLOSING RDG</th>
                             <th>MF</th>
                              <th>REC_KVAH</th>
					</tr>
				</thead>
					...
					<c:forEach var="mtrblc" items="${trueup}"  varStatus="tagStatus">
							<tr>
						       <td>${tagStatus.index + 1}</td>
								...
								<td>${mtrblc.USCNO}</td>
								<td>${mtrblc.CTNAME}</td>
								<td>${mtrblc.OPEN_DT}</td>
								<td>${mtrblc.END_DT }</td>
								<td>${mtrblc.KVAH_OP_READING}</td>
                                <td>${mtrblc.KVAH_CLOS_RDG}</td>
                                 <td>${mtrblc.MF}</td>
