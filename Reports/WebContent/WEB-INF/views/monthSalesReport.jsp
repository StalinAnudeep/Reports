...
<div class="row row-cards row-deck">
	<form class="card" action="monthSalesReport" method="post">
		<div class="card-body">
			<h3 class="card-title">
				<strong><span class="text-danger">HT130 </span> -
					Month Wise,Category Wise,Sales Report </strong>
			</h3>
			<div class="row">
				...
				<div class="col-md-6">
					<div class="form-group">
						<label class="form-label"> Get Month Sales Report
						</label>
						<button type="submit" class="btn btn-primary">Get
							Month Sales Report</button>
					</div>
				</div>
			</div>
		</div>
				...
				<form name="frm" style="overflow: auto;">
					...
					<table id="multiLevelTable"
						...
						<tbody>
							...
							String circletype = "S";
							%>
							<c:forEach var="sd" items="${monthSalesDetails}" varStatus="tagStatus">
						...
						<tfoot>
							<tr class="bg-primary text-light">
								...
								<td class="text-center format">${monthSalesDetails.stream().map(sd -> sd.SCS).sum()}</td>
								<td class="text-center format">${monthSalesDetails.stream().map(sd -> sd.CAPACITY).sum()}</td>
