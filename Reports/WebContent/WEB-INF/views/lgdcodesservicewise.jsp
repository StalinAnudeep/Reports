...
<div class="row row-cards row-deck">
	<form class="card" action="lgdcodesservicewise" method="post">
		<div class="card-body">
			<h3 class="card-title"><strong><span class="text-danger">HT96A</span> - LGD Service Wise Report</strong></h3>
			<div class="row">
				<div class="col-md-3">
				...
				<div class="col-md-3">
					<div class="form-group">
						<label class="form-label">GET LGD </label>
						<button type="submit" class="btn btn-success">GET LGD </button>
					</div>
				</div>
			</div>
		</div>
	...
	<c:if test="${ not empty fn:trim(acd)}">
		<div class="card ">
			<div
				...
				<table
					class="table card-table table-vcenter text-nowrap datatable display" width="100%"
					>
					<thead>
						<tr>
							<th>S.NO</th>
					...
					<tbody>
						<c:forEach var="mtrblc" items="${acd}" varStatus="tagStatus">
							<tr>
								...
								<td>${mtrblc.USCNO}</td>
								<td>${mtrblc.DISTRICT}</td>
								<td>${mtrblc.NAME}</td>
								<td>${mtrblc.NAME_TYPE}</td>
								<td>${mtrblc.LGDCODE}</td>
								<td>${mtrblc.DDOCODE}</td>
								<td>${mtrblc.CMFSCODE}</td>
								<td>${mtrblc.MONTH}</td>
