...
public class GenerateDMDEVICECTPT {
	...
	private static void printPageOne(PrintStream ps,Object o) {
		...
		System.out.println(d.get("CTSUPCONDT").toString().trim());
		ps.println(d.get("USCNO").toString().trim().replace(" ", "")
				 ...
				 +"\t"+Optional.ofNullable(d.get("ACQ_DT")).orElse("")//ANSDT Acquisition date
...
*/				);
		ps.println(d.get("USCNO").toString().trim().replace(" ", "")
				+"\tEGERH"
/*				+"\t99991231" //BIS
*/				+ "\t"+d.get("VALIDFROMT").toString().trim()///AB ACT_COM_DT
				);
		ps.println(d.get("USCNO").toString().trim().replace(" ", "")
				+"\tCLHEAD"
				+"\tZCTPT_SET_SEALS"//CLASS
				+"\tIS2"// CLASSTYPE
				);
		ps.println(d.get("USCNO").toString().trim().replace(" ", "")
				+"\tCLDATA"
